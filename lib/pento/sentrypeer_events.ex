defmodule Pento.SentrypeerEvents do
  @moduledoc """
  ...
  def create_sentrypeer_event(attrs \\ %{}) do
    ...
    |> Repo.insert()
    |> broadcast(:sentrypeer_event_created)
  end
  ...
  def subscribe do
    Phoenix.PubSub.subscribe(Pento.PubSub, "sentrypeer_events")
  end

  defp broadcast({:error, _reason} = error, _event), do: error

  defp broadcast({:ok, sentrypeer_event}, event) do
    IO.inspect(sentrypeer_event)
    IO.inspect(event)
    Phoenix.PubSub.broadcast(Pento.PubSub, "sentrypeer_events", {event, sentrypeer_event})
    {:ok, sentrypeer_event}
  end
end
